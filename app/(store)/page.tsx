...
export default function Home() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setDbProducts(productsData as StoreProduct[]);
      setDbSettings(settingsData as Record<string, string>);
    }
    fetchData();
  }, []);

  // Best seller products (filtered)
  const bestSellers = dbProducts.filter(product => product.featured).slice(0, 4);

  const heroBadgeText = dbSettings.hero_badgeText || dbSettings.heroBadgeText || 'Toko Koin & Cues Terpercaya No. 1';
  const heroTitle = dbSettings.hero_title || dbSettings.heroTitle || 'Jual Item 8 Ball Pool Terpercaya';
  const heroSubtitle =
    dbSettings.hero_subtitle ||
    dbSettings.heroSubtitle ||
    'Dapatkan Coins, Cash, Legendary Cue, Cue Pieces, dan item premium 8 Ball Pool dengan harga terbaik dan proses secepat kilat.';
  const heroButtonText = dbSettings.hero_buttonText || dbSettings.heroButtonText || 'Lihat Produk';
  ...
  const howToOrderSteps = (() => {
    const parsed = parseJsonSafely<unknown[]>(dbSettings.how_to_order, DEFAULT_HOW_TO_ORDER);
    const normalized = normalizeHowToOrder(parsed);
    return normalized.length > 0 ? normalized : DEFAULT_HOW_TO_ORDER;
  })();

  const faqItems = (() => {
    const parsed = parseJsonSafely<unknown[]>(dbSettings.faqs, DEFAULT_FAQS);
    const normalized = normalizeFaqs(parsed);
    return normalized.length > 0 ? normalized : DEFAULT_FAQS;
  })();

  const testimonialItems = (() => {
    const parsed = parseJsonSafely<unknown[]>(dbSettings.testimonials, DEFAULT_TESTIMONIALS);
  ...
  // Categories list
  const categories = [
    { id: 'coins', name: 'Coins', icon: Coins, count: '4 Items' },
    { id: 'cash', name: 'Cash', icon: Sparkles, count: '3 Items' },
    { id: 'cues', name: 'Legendary Cue', icon: Award, count: '3 Items' },
    { id: 'pieces', name: 'Cue Pieces', icon: Layers, count: '2 Items' },
    { id: 'events', name: 'Event Items', icon: ShieldCheck, count: '2 Items' },
    { id: 'bundles', name: 'Special Bundles', icon: Gift, count: '2 Items' }
  ];
        ...
        >
          ...
          <div className="flex flex-col items-center text-center lg:items-start lg:text-left lg:w-1/2">
            ...
            <motion.div variants={itemVariants} className="mt-10 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              ...
              <a
                href={dbSettings.whatsapp ? `https://wa.me/${dbSettings.whatsapp}` : "https://wa.me/6281234567890"}
                target="_blank"
          ...
          >
            ...
            >
              ...
              <Image
                src={getSafeImageSrc(dbSettings.hero_image2_url, '/cues/firestorm.png')}
                alt="Hero Card 2"
                fill
              ...
              <div className="absolute bottom-5 left-5 z-20 flex flex-col pointer-events-none">
                <span className="text-sm font-black tracking-widest text-white uppercase drop-shadow-lg">
                  {dbSettings.hero_image2_label || 'Firestorm'}
                </span>
              </div>
              {swapped && (
            ...
            >
              ...
              <Image
                src={getSafeImageSrc(dbSettings.hero_image1_url, '/cues/archangel.png')}
                alt="Hero Card 1"
                fill
                priority
                sizes="(max-width: 768px) 280px, 380px"
              ...
              <div className="absolute bottom-5 left-5 z-20 flex flex-col pointer-events-none">
                <span className="text-[9px] font-bold text-cyan-glow uppercase tracking-wider mb-0.5 drop-shadow-md">
                  {dbSettings.hero_image1_sublabel || 'Mythic Tier'}
                </span>
                <span className="text-sm font-black tracking-widest text-white uppercase drop-shadow-lg">
                  {dbSettings.hero_image1_label || 'Archangel'}
                </span>
              </div>
              {!swapped && (
                <div className="absolute top-4 right-4 h-2.5 w-2.5 rounded-full bg-cyan-glow animate-pulse shadow-[0_0_12px_#00E5FF] pointer-events-none"></div>
      ...
      <section className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 border-t border-dark-purple/30">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5">
          {categories.map((cat, idx) => {
            ...
            return (
              <FadeIn key={cat.id} delay={0.1 * idx} direction="up">
                <Link 
      ...
      <section className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 border-t border-dark-purple/30">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {bestSellers.map((product, idx) => (
            <FadeIn key={product.id} delay={0.1 * idx} direction="up">
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-8 xl:gap-4 relative z-10">
            {howToOrderSteps.map((stepObj, idx, arr) => {
              ...
              return (
                <FadeIn key={`${stepObj.title}-${idx}`} delay={0.1 * idx} direction="up">
                  <div className="flex flex-col items-center text-center gap-5 group relative">
                    <div className="relative">
                      {/* Step Number Circle */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonialItems.map((test, index) => (
            <FadeIn key={index} delay={0.1 * index} direction="up">
              <div className="premium-card rounded-xl p-5 flex flex-col gap-4 text-left h-full">
          ...
          {faqItems.map((faq, index) => {
            ...
            return (
              <FadeIn key={index} delay={0.1 * index} direction="up" fullWidth>
                <div className="premium-card rounded-xl overflow-hidden border-dark-purple/35 w-full">
                  <button
                  ...
                  {isOpen && (
                    <div className="px-4.5 pb-4.5 pt-0.5 border-t border-dark-purple/20 text-[11px] text-gray-text leading-relaxed bg-[#09080e]/40 animate-in fade-in duration-200 text-left">
                      {faq.answer}
                    </div>
                  )}
                </div>
